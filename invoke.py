# =============================================================================
# invoke.py — generic "invoke anything anywhere" layer
# =============================================================================
# Promoted from tests/dynamic_invoke.py — these stopped being test-only
# utilities once the debug GUI's Test Runner panel needed them live, to
# invoke any function or workflow directly from a running desktop app, not
# just from pytest. tests/dynamic_invoke.py now re-exports from here so
# existing tests keep working unchanged.
#
# Two capabilities, both deliberately avoiding static imports so callers keep
# working against code that didn't exist when this module was written:
#
...
#       module was written is invocable through this with zero changes,
...
# the point, it's a generic capability — but don't call it from a test that
# runs automatically in a normal suite, and the debug GUI's Test Runner
# surfaces a warning before running one of these by name.
# =============================================================================
...
def workflow_registry() -> dict:
    ...
    currently in workflows/, including files that didn't exist when any
    caller importing this was written.
    """
